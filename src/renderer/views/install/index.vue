...
export default {
  ...
  computed: {
    ...mapState({
      macAddress: state => state.pos.macAddress
    })
  },
  mounted() {
  ...
  methods: {
    ...
    initConfig() {
      ...
      Object.keys(this.ruleForm).forEach(key => {
        this.$store.dispatch('pos/changeSetting', { key, value: this.ruleForm[key] })
      })
    },
    // 完成
    complete() {
      Object.keys(this.ruleForm).forEach(key => {
        this.$store.dispatch('pos/changeSetting', { key: 'install', value: true })
      })
